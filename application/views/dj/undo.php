...
<style type="text/css">.wj_t{ width: 400px;}</style>
...
$(document).ready(function(e) {
    ...
    var url = baseUrl+'Dj/listdata?zd=2';
	getPage(1, url);
});
...
function showInfo(val){
	...
	for(var i=0;i<val.data.length;i++){
		...
		tbody +=
			'<td>'+getLocalTime(val.data[i]['q_end'])+((val.data[i]['nowtime']>(Number(val.data[i]['q_start'])))?((val.data[i]['nowtime']<=(Number(val.data[i]['q_end'])+(3600*24)))?'[运行中]':'[结束]'):'[未开始]')+'</td>'
			+'<td>'+eval("type_array["+val.data[i]['fk_type_id']+"]")+'</td>'
			+'<td>'+val.data[i]['c_userid']+'</td>'
			+'<td>'+val.data[i]['dj_num']+'</td>'
			+'<td><a href="'+baseUrl+'Dj/start/wjid/'+val.data[i]['id']+'/" target="_blank">答卷</a></td>';
