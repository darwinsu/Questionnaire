...
var page='subType';//定义当前页面
function sytle_show(){
	var temp=do_ajax('getSytleData','t=3'); 
	showInfo(temp,'');		
}
$(document).ready(function(e) {
	...
	$('#q_edit_save').live('click',function(){
		var temp_val="t=3&name="+$('#ebodys #ebody #name').val()+"&qid="+$('#qid').val();
	...
	$('#search_item_btn').live('click',function(){	
		 var temp=do_ajax('getSytleData','t=3&name='+$('#search_item_input').val()); 
		 showInfo(temp,'');	
	});
	//添加	 
	$('#q_add_save').live('click',function(){
		var temp_val="t=3&&name="+$('#ebodys #ebody #name').val();
