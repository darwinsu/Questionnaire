...
function fileQueued(file) {
	try {
		if(up_temp_num>0){//判断当前文件数目
		alert("您选择上传的文件超过最大上传限制!"); 
		}else{
...
function fileQueueError(file, errorCode, message) {
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			alert("您选择上传的文件超过最大上传限制！.\n" + (message === 0 ? "You have reached the upload limit." : "您现在最多只能选择上传" + (message > 1 ?  message + "个文件." : "一个文件.")));
			return;
		}
		...
		case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
			...
			progress.setStatus("图片太大，请压缩后再上传.");
...
function uploadSuccess(file, serverData) {
	try {
        ...
        if (serverData.substring(0, 7) === "FILEID:") {
			 initthumselect(serverData.substring(7),file.id,file.name,file.size);
